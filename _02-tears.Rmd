...
The same is basically true for the functions `labelled::val_labels()`, `sjlabelled::get_labels()`, and `expss::val_lab()`. All three functions call `attr(x, "labels", extract = TRUE)`. The function `sjlabelled::get_labels()` further processes the attributes, resulting in a slightly different output than `labelled::val_labels()` and `expss::val_lab()`. Ultimately, as long as the object's value labels are stored in an attribute called `"labels"`, any of these functions will return the value labels.
...
The reason behind `readstata13::get.label()` returning `NULL` is similar. Let's look at the function.
